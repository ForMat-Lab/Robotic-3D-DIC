...
def capture_images(cameras, num_samples, output_folder, experiment_name, experiment_description, exposure_time, arduino_controller, trigger_pin):
    # Record start time
        ...
        if arduino_controller.check_rising_edge(trigger_pin):
            ...
            if all(camera.IsGrabbing() for camera in cameras):
                ...
                if all(result.GrabSucceeded() for result in results):
                    ...
                    # Resize frames for display and concatenate them side by side
                    resized_frames = [cv2.resize(frame, (0, 0), fx=0.5, fy=0.5) for frame in frames]  # Adjust the scale as needed
                    concatenated_frame = cv2.hconcat(resized_frames)

                    # Display the concatenated frame in a single window
...
if __name__ == "__main__":
    ...
    try:
        ...
        capture_images(cameras, num_samples, output_folder, experiment_name, experiment_description, exposure_time, arduino_controller, trigger_pin)
